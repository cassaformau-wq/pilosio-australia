    ...
    <div>
      ...
      <div className="bg-[#023569] text-white py-12 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-white text-xs font-bold uppercase tracking-widest mb-2">{category}</div>
      ...
      <div className="relative h-64 md:h-96 overflow-hidden">
        ...
        {highlight && (
          <div className="absolute top-6 left-6 bg-[#023569] text-white text-sm font-bold px-4 py-2 uppercase tracking-wider">
            {highlight}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            {characteristics?.length > 0 && (
              <div>
                <h2 className="text-[#023569] text-2xl font-black uppercase tracking-wide mb-6 flex items-center gap-3">
                  <img src={RED_P} alt="P" className="h-7 w-auto" /> Characteristics
                </h2>
                <ul className="space-y-3">
                  {characteristics.map((c, i) => (
                    <li key={i} className="flex items-start gap-3 text-gray-600 leading-relaxed">
                      <CheckCircle className="w-5 h-5 text-[#023569] flex-shrink-0 mt-0.5" />
                      <span>{c}</span>
                    </li>
                  ))}
                </ul>
            ...
            {advantages?.length > 0 && (
              <div>
                <h2 className="text-[#023569] text-2xl font-black uppercase tracking-wide mb-6 flex items-center gap-3">
                  <img src={RED_P} alt="P" className="h-7 w-auto" /> Advantages
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {advantages.map((a, i) => (
                    <div key={i} className="border-l-4 border-[#023569] pl-4 py-2 bg-[#f5f7fa]">
            ...
            {applications?.length > 0 && (
              <div>
                <h2 className="text-[#023569] text-2xl font-black uppercase tracking-wide mb-6 flex items-center gap-3">
                  <img src={RED_P} alt="P" className="h-7 w-auto" /> Applications
                </h2>
                <div className="flex flex-wrap gap-2">
                  {applications.map((a, i) => (
                    <span key={i} className="border border-[#023569] text-[#023569] text-xs font-bold uppercase tracking-wider px-3 py-2">
                      {a}
                    </span>
                  ))}
                </div>
          ...
          <div className="space-y-8">
            ...
            {technicalData?.length > 0 && (
              <div className="bg-[#023569] text-white p-6">
                <h3 className="font-black uppercase tracking-wide text-sm mb-5 text-white">Technical Data</h3>
                <ul className="space-y-3">
            ...
            <div className="bg-[#f5f7fa] p-6 border-l-4 border-[#023569]">
              <h3 className="font-black uppercase tracking-wide text-sm text-[#023569] mb-2">Request a Quote</h3>
              <p className="text-gray-500 text-sm mb-4">Get in touch with our Australian team for pricing and availability.</p>
              <Link
                to={createPageUrl("Contact")}
                className="flex items-center gap-2 bg-[#023569] text-white px-5 py-3 font-bold uppercase tracking-wider text-sm hover:bg-[#012a52] transition-colors"
              >
            ...
            {relatedProducts?.length > 0 && (
              <div>
                <h3 className="font-black uppercase tracking-wide text-sm text-[#023569] mb-4">Related Products</h3>
                <ul className="space-y-2">
                  {relatedProducts.map((r) => (
                    <li key={r.page}>
                      <Link
                        to={createPageUrl(r.page)}
                        className="flex items-center justify-between border border-gray-200 hover:border-[#023569] p-3 group transition-colors"
                      >
                        <span className="text-[#023569] text-sm font-semibold group-hover:text-[#012a52] transition-colors">{r.name}</span>
                        <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-[#012a52]" />
                      </Link>
                    </li>
                  ))}
                </ul>
      ...
      {/* Bottom CTA */}
      <section className="bg-[#023569] text-white py-12 text-center px-4">
